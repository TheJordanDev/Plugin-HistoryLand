...
public class BroadcastCommand extends AbstractCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length < 3) {
            return sendMessageF(sender,Historyland.GAME_PREFIX+"§c/hbroadcast <chat/action> <langue> <message/code>");
        } else {

            LanguageManager.BCType type;
            Optional<LanguageManager.BCType> msgType = Helper.isValueOfEnum(LanguageManager.BCType.class, args[0].toUpperCase());
            ...
            Optional<Language> language = LanguageManager.instance().getLanguageFromName(args[1]);
            if (language.isEmpty()) {
                if (args[1].equals("all") || args[1].equals("*")) lang = LanguageManager.BCLanguage.ALL;
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        else if (args.length == 1) autocompleted = Arrays.stream(LanguageManager.BCType.values()).map(LanguageManager.BCType::name).collect(Collectors.toList());
        else if (args.length == 2) autocompleted = Helper.addToList(LanguageManager.instance().languages().values().stream().map(Language::name).collect(Collectors.toList()),"*","all");
